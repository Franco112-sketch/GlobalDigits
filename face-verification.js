...
/* ── Motion detection, rewritten ──────────────────────────────────────
   The previous version sampled frame-to-frame deltas for the whole
   SAMPLE_WINDOW_MS window and only checked the AVERAGE delta once the
   window finished. That average is diluted by every still frame before
   and after the actual turn — a normal, fairly quick head turn only
   occupies a fraction of the window, so its real motion could get
   averaged down below MOTION_THRESHOLD even though the turn clearly
   happened. That's what produced "I turned and it still said it
   couldn't detect movement".

   This version instead checks each frame-to-frame delta AS IT ARRIVES
   and counts the step as passed the moment any single delta clears the
   threshold — no waiting for the full window, no averaging away a
   real, brief motion. The window is now only a timeout: if NO frame
   ever clears the threshold before it elapses, the step is retried. To
   avoid one noisy/jittery frame counting as a false pass, two
   consecutive over-threshold samples are required, which is quick
   (well under SAMPLE_INTERVAL_MS * 2 ≈ 240ms) for a genuine turn but
   filters out a single camera glitch. */
function runStep(){
  if(FV.stepIndex>=STEPS.length){ finishSuccess(); return; }
  updateStepUI();
  FV.lastFrame = null;
  FV.consecutiveHits = 0;
  FV.sampleTimer = setInterval(function(){
    var frame = sampleGrayFrame(FV.video);
    if(FV.lastFrame && frame){
      var delta = frameDelta(frame, FV.lastFrame);
      if(delta >= MOTION_THRESHOLD){
        FV.consecutiveHits++;
        if(FV.consecutiveHits >= 2){
          stepPassed();
  ...
  FV.stepTimer = setTimeout(stepFailed, SAMPLE_WINDOW_MS);
}
function stepPassed(){
  if(FV.sampleTimer){ clearInterval(FV.sampleTimer); FV.sampleTimer=null; }
  if(FV.stepTimer){ clearTimeout(FV.stepTimer); FV.stepTimer=null; }
