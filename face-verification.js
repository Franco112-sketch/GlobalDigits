/* ══════════════════════════════════════════════════════════════════
   ...
   right, up and down in front of the camera. If every step passes,
   it shows a short success state and calls onVerified(). If the user
   backs out at any point, it calls onCancelled() instead and nothing
   ...
   biometric identity verification, and nothing in this file or its
   UI text should ever be changed to claim otherwise.
...
var MOTION_THRESHOLD = 14;     // avg per-pixel grayscale delta needed to count as "moved"
...
+'.fvBody{flex:1;overflow-y:auto;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:20px 20px 28px;text-align:center;}'
+'.fvFrameWrap{position:relative;width:168px;height:168px;margin:0 auto 18px;}'
+'.fvFrameRing{position:absolute;inset:0;border-radius:50%;border:2.5px solid var(--brand,#1a56db);opacity:.9;transition:border-color .25s ease;}'
+'.fvFrameRing.ok{border-color:#16a34a;}'
+'.fvFrameRing.bad{border-color:var(--danger,#ef4444);}'
+'.fvVideoCircle{position:absolute;inset:7px;border-radius:50%;overflow:hidden;background:#0d1321;display:flex;align-items:center;justify-content:center;}'
+'.fvVideoCircle video{width:100%;height:100%;object-fit:cover;transform:scaleX(-1);}'
+'.fvVideoCircle .fvPlaceholder{color:#fff;opacity:.55;}'
...
+'.fvProgress{font-size:10.5px;color:var(--text2,#5a6580);font-weight:600;margin:14px 0 4px;}'
...
/* ══════════════════════════════════════════════════════════════════
   3. MARKUP (injected once, lazily, on first start())
   ══════════════════════════════════════════════════════════════════ */
function buildOverlay(){
  ...
  +'  </div>'
  +'  <div class="fvBody" id="fvBody"></div>'
  +'</div>';
...
function renderShell(){
  var body=$('fvBody'); if(!body)return;
  body.innerHTML =
    '<div class="fvFrameWrap">'
    +'  <div class="fvFrameRing" id="fvRing"></div>'
    +'  <div class="fvVideoCircle" id="fvVideoWrap"><span class="fvPlaceholder">'+svgCamera()+'</span></div>'
    ...
    +'<div class="fvDirRow" id="fvDirRow"></div>'
...
/* Per-step text/icon refresh only — deliberately never touches
   #fvVideoWrap, so the live <video> element that openCamera() drops
   in there stays put across every step instead of being wiped back
   to the placeholder camera icon on each re-render. */
function updateStepUI(){
  ...
  var dirRow=$('fvDirRow');
  if(dirRow){
    dirRow.innerHTML = STEPS.map(function(s,i){
      var cls = i<FV.stepIndex ? 'done' : (i===FV.stepIndex ? 'active' : '');
      var icon = i<FV.stepIndex ? svgCheck(14,'currentColor') : svgArrow(s.key);
      return '<div class="fvDirBtn"><div class="fvDirIcon '+cls+'">'+icon+'</div><span>'+s.label.replace('Look ','')+'</span></div>';
    }).join('');
  }
...
function renderPermissionError(message){
  ...
  body.innerHTML =
    ...
    +'<div class="fvSub">'+(message||'Please allow camera access to verify it\'s you.')+'</div>'
    +'<button class="fvBtn fvBtnP" id="fvRetryBtn">Try Again</button>'
    +'<button class="fvBtn fvBtnO" id="fvCancelBtn">Back</button>';
  $('fvRetryBtn').addEventListener('click', function(){ beginFlow(); });
  $('fvCancelBtn').addEventListener('click', function(){ cancelFlow(); });
}
...
function renderFailure(){
  ...
  body.innerHTML =
    ...
    +'<button class="fvBtn fvBtnP" id="fvRetryBtn">Try Again</button>'
    +'<button class="fvBtn fvBtnO" id="fvCancelBtn">Cancel</button>';
  $('fvRetryBtn').addEventListener('click', function(){ beginFlow(); });
  $('fvCancelBtn').addEventListener('click', function(){ cancelFlow(); });
}

/* ══════════════════════════════════════════════════════════════════
   4. CAMERA + BASIC MOTION-BASED LIVENESS
   ══════════════════════════════════════════════════════════════════ */
var canvas, ctx;
...
async function openCamera(){
  ...
  var wrap=$('fvVideoWrap');
  if(wrap){ wrap.innerHTML=''; wrap.appendChild(video); }
  FV.video = video;
  return video;
}
function stopCamera(){
...
function start(onVerified, onCancelled){
  if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){
    ...
    FV.onVerified=onVerified; FV.onCancelled=onCancelled;
    renderPermissionError('Your browser doesn\'t support camera access. Please try a different browser.');
    return;
  }
  buildOverlay();
